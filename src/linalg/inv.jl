...
function materialize!(M::BlasMatLdivVec{<:TriangularLayout{'U','N'}})
    A,b = M.A,M.B
...
function materialize!(M::BlasMatLdivVec{<:TriangularLayout{'U','U'}})
    A,b = M.A,M.B
    require_one_based_indexing(A, b)
    n = size(A, 2)
    if !(n == length(b))
...
function materialize!(M::BlasMatLdivVec{<:TriangularLayout{'L','N'}})
    A,b = M.A,M.B
    require_one_based_indexing(A, b)
    n = size(A, 2)
    if !(n == length(b))
